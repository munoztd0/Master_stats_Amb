...
base_price = filter(base_clean, id %notin% c("11188", "12775", "13105", "12721")) #remove huge outliers, see appendix
...
plotNormalHistogram(base_clean$Reccomend, main = "Reccomendation", sub = paste("skewness =", skewness(base_clean$Reccomend, na.rm = TRUE)))



plotNormalHistogram(base_clean$Buy, main = "Probability to buy", sub = 
paste("skewness =", skewness(base_clean$Buy, na.rm = TRUE)))


plotNormalHistogram(base_clean$Ecolo_Eval, main = "Ecologic Evaluation", sub =
paste("skewness =", skewness(base_clean$Ecolo_Eval, na.rm = TRUE)))
...
#### Diagnostics Plots for GLM Price with outliers

```{r diag GLM Price, echo=FALSE, message=FALSE, warning=FALSE, fig.show="hold", out.width="50%"}
...
#### Diagnostics Plots for GLM Price without outliers

```{r diag GLM Price2, echo=FALSE, message=FALSE, warning=FALSE,  fig.show="hold", out.width="50%"}
...
```{r diag GLM MSprice, echo=FALSE, message=FALSE, warning=FALSE}

MS = MASS::stepAIC(modprice, direction = "both", trace = FALSE)

x = kable(attributes(MS$anova)$heading, format="html") %>%
    kable_styling(latex_options = "HOLD_position", position = "center", full_width = F) %>%  row_spec(1, bold=T,align='c') %>%  row_spec(4, bold=T)
gsub("<thead>.*</thead>", "", x)
...
#### Diagnostics Plots for GLM Probability to buy with outliers
```{r diag GLM Buy, echo=FALSE, message=FALSE, warning=FALSE, fig.show="hold", out.width="50%"}
...
#### Diagnostics Plots for GLM Probability to buy without outliers
```{r diag GLM Buy2`, echo=FALSE, message=FALSE, warning=FALSE, fig.show="hold", out.width="50%"}
...
```{r diag GLM MSbuy, echo=FALSE, message=FALSE, warning=FALSE}
